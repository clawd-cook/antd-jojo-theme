...
Bold thick black outlines, sculptural anatomy, exaggerated pose tension, and vibrant non-naturalistic clash color — Hirohiko Araki / *JoJo’s Bizarre Adventure* as UI. Drop into `ConfigProvider` for magazine-cover tension instead of soft SaaS chrome.
...
- **Hard ink shadows** — comic offsets (`3px` / `5px` / `8px`), never soft blur
...
- **Zero border radius** — chiselled geometry; focus rings off (`controlOutlineWidth: 0`)
...
Pass `jojoTheme` and (recommended) import the chrome CSS:
...
Manual hatch / shadow without the CSS file:

```tsx
import { jojoColors, jojoHatch, jojoInkShadow } from "@clawd-cook/antd-jojo-theme";
...
| `jojoInkShadow` | `string` | Hard offset `5px 5px 0` |
| `jojoInkShadowSm` | `string` | `3px 3px 0` |
| `jojoInkShadowLg` | `string` | `8px 8px 0` |
| `jojoMagentaShadow` / `jojoGoldShadow` | `string` | Jewelry / menace accent shadows |
| `jojoHatch` | `string` | Crosshatch `backgroundImage` |
| `jojoHatchDense` | `string` | Denser hatch for dark rails |
...
> [!TIP]
> Prefer `jojoInkShadow*` over blurred `box-shadow`. Depth here is a stamped comic offset, not ambient elevation.
